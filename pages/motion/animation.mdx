...
However, you can set different types of animation by passing a [`Transition` to the `transition` prop](/legacy/api/motion/types#transition).
...
The `useAnimation` hook can be used to create a set of [imperative `AnimationControls`](/legacy/api/motion/types/#animationcontrols) with a `start` and `stop` method. These controls can be passed to **one or more** `motion` components via the `animate` prop.
